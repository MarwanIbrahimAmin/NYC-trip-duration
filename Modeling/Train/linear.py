...
class Train():
    def __init__(self):
        self.reg = LinearRegression(fit_intercept=config['Model']['LinearRegression']['fit_intercept'])
...
if __name__ == '__main__':
    ...
    train_pred = trainer.reg.predict(X_train)
    val_pred = trainer.reg.predict(X_val)
    ...
    model_params = {
        "fit_intercept": config['Model']['LinearRegression']['fit_intercept']
    }
    ...
    coefs = trainer.reg.coef_
    feature_importance = {name: float(coef) for name, coef in zip(feature_names, coefs)}
    feature_importance["_intercept"] = float(trainer.reg.intercept_)
    ...
    model_save_path = os.path.join(config['save_paths']['models_dir'], "linearRegression_model.joblib")
    save_object(trainer.reg, model_save_path)
