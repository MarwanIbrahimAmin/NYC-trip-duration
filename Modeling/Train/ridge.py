...
class Train():
    def __init__(self):
        self.Ridge = Ridge(alpha=config["Model"]['Ridge']['alpha'],
                           fit_intercept=config['Model']['Ridge']['fit_intercept'],
                           solver=config["Model"]['Ridge']['solver'],
                           positive=config["Model"]['Ridge']['positive'],
...
if __name__ == '__main__':
    ...
    train_pred = trainer.Ridge.predict(X_train)
    val_pred = trainer.Ridge.predict(X_val)
    ...
    model_params = config['Model']['Ridge'].copy()
    ...
    coefs = trainer.Ridge.coef_
    feature_importance = {name: float(coef) for name, coef in zip(feature_names, coefs)}
    feature_importance["_intercept"] = float(trainer.Ridge.intercept_)
    ...
    model_save_path = os.path.join(config['save_paths']['models_dir'], "ridge_model.joblib")
    save_object(trainer.Ridge, model_save_path)
